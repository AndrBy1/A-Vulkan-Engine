...
namespace std{
    ...
    template<> //in this case template is for specializing the std::hash struct for a specific type, which is mve::MveModel::Vertex
...
namespace ave
{
    ...
    VkDescriptorImageInfo AveModel::attachTextureFromFile(const std::string& filepath) {
    ...
    void AveModel::createVertexBuffers(const std::vector<Vertex>& vertices) {
        ...
        vertexBuffer = std::make_unique<AveBuffer>(aveDevice, vertexSize, vertexCount,
    ...
    void AveModel::createIndexBuffers(const std::vector<uint32_t>& indices) {
        ...
        indexBuffer = std::make_unique<AveBuffer>(aveDevice, indexSize, indexCount,
