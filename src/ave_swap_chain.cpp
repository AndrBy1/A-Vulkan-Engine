...
namespace ave {
  ...
  VkResult AveSwapChain::submitCommandBuffers(
    ...
    //vkQueueWaitIdle(device.presentQueue()); // HELPS BUT NOT OPTIMAL!
  ...
  void AveSwapChain::createSwapChain() {
    ...
    // ? is this else otherwise, if oldSwapChain == nullptr then oldSwapchain = VK_NULL_HANDLE otherwise oldSwapChain->swapChain
  ...
  /*
  void AveSwapChain::createImageView(VkImage image, VkFormat format, int i){
    ...
    if(i == -1){ //if this is for a non swap chain image view
  ...
  VkSurfaceFormatKHR AveSwapChain::chooseSwapSurfaceFormat(
  ...
  //FIFO is where swap chain syncs with vsync of disply
  //Mailbox is where swap chain syncs with vsync of display, but allows for triple buffering, this lowers latency but consumes more gpu
  //Immediate is where swap chain does not sync with vsync, this can lead to tearing but reduces latency
  VkPresentModeKHR AveSwapChain::chooseSwapPresentMode(
  ...
  VkFormat AveSwapChain::findDepthFormat() {
